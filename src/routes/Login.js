...
import { showPassword, handleToggle, validateEmail } from '../utils/functions';
...
function Login() {
    ...
    const handleEmailBlur = () => {
        setIsErr(prev => ({ ...prev, email: validateEmail(data.email) }))
    };
    const handlePasswordBlur = () => {
        setIsErr(prev => ({ ...prev, password: !data.password }))
    };
    ...
    const handleLogin = async () => {
        await login(data, setIsLoading, isErr, navigate);
    }
                ...
                <div className='w-full bg-black md:w-[750px] md:rounded-md md:mt-9 flex flex-col md:items-center md:justify-center'>
                    ...
                    <div className='w-full py-10 border-b border-gray-800 md:w-1/2'>
                        <div className='flex flex-col gap-4'>
                            <a href='http://localhost:8080/auth/google/login' className='flex flex-row items-center gap-6 py-3 px-8 rounded-3xl border border-gray-500 border-solid cursor-pointer'>
